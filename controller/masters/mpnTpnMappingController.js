...
const performMpnTpnMappingAction = async (req, res) => {
    const { action, method } = req.body;

    try {
        const response = await executeSP({
            spName: 'MPNTPNMapping',
        ...
        if (['Add', 'Upload', 'Update', 'Delete', 'StatusUpdate'].includes(action)) {
            ...
            return res.status(201).send({ notify: false, success, data, message: 'success', data, action, method });
        }
        if (action === 'CheckTPNInPart') {
            if (error) return res.status(201).send({
                notify: true,
                success: false,
                error: true,
...
export { performMpnTpnMappingAction }