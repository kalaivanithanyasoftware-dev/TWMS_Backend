...
const performMaterialConsumption653Action = async (req, res) => {
    const { action, method } = req.body;
    try {
        const response = await executeSP({
            spName: 'MaterialConsumption653',
        ...
        if (action === 'MaterialConsumption653') {
...
export { performMaterialConsumption653Action }