...
const performDockAllocationActions = async (req, res) => {
    const { action, method } = req.body;
    try {
        const response = await executeSP({
            spName: 'DockAllocation',
        ...
        if (['Add', 'Update', 'Delete', 'StatusUpdate', 'DockIn', 'DockOut'].includes(action)) {
        ...
        if (['GetTruckNumbers', 'GetDockList', 'GetSecuritySerialNumbers'].includes(action)) {
            return res.status(201).send({ notify: false, success, message: 'success', data, action, method });
        }
        if (action === 'Fetch') {
...
export { performDockAllocationActions }