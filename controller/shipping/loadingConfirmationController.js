...
const performLoadingConfirmationActions = async (req, res) => {
    const { action, method } = req.body;

    try {
        const response = await executeSP({
            spName: 'LoadingConfirmation',
        ...
        if (['UpdateLoadingConfirmation'].includes(action)) {
...
export {
    performLoadingConfirmationActions
}