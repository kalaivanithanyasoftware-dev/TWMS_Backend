...
const performCMRequestAction = async (req, res) => {
    ...
    try {
        ...
        if (['Upload', 'Bypass'].includes(action)) {
            if (error) {
        ...
        if (['GetPartTypes', 'SuppliersList', 'StorageLocations', 'PlantCodeList'].includes(action)) {
            return res.status(201).send({ notify: false, success, data, action, method });
        }
