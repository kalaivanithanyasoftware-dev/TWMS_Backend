...
const performQuarantineInventoryAction = async (req, res) => {
    const { action, method } = req.body;

    try {
        const response = await executeSP({
            spName: 'QuarantineInventory',
        ...
        if (['MoveToQuarantineInventory', 'MoveToInventory'].includes(action)) {
            if (error) {
        ...
        if (['Locations', 'MixedLocations', 'QuarantineLocations'].includes(action)) {
...
export { performQuarantineInventoryAction }