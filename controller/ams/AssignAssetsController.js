...
const performAssignAssetsAction = async (req, res) => {
    ...
    try {
        ...
        const { success, error, results: [data = [], [{ count: totalRecords = 0 } = {}] = []] } = (await executeSP([
            {
                ...
                params: [
                    ...
                    { name: 'grir_Name', type: sql.BigInt, value: grir_Name },
                    ...
                    { name: 'excelArray', type: sql.NVarChar, value: excelFileData },
        ...
        if (['Add', 'Update', 'Upload', 'Delete', 'StatusUpdate', 'Assign', 'Scrap'].includes(action)) { 
        ...
        if (['VendorsList', 'PlantCodeList', 'AssetCategoryList', 'AssetTypeList', 'BrandsList', 'EmployeeList'].includes(action)) {
