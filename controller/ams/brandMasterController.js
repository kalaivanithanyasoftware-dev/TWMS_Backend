...
const performBrandMasterAction = async (req, res) => {
    ...
    try {
        ...
        const { success, error, results: [data = [], [{ count: totalRecords = 0 } = {}] = []] } = (await executeSP([
            {
                ...
                params: [
                    ...
                    { name: 'brandname', type: sql.NVarChar, value: brandname },
        ...
        if (['Add', 'Update', 'Upload', 'Delete', 'StatusUpdate'].includes(action)) {
            ...
            return res.status(201).send({ notify: false,data, success, message: 'success', action, method });
        }
        if (action === 'Export') {
