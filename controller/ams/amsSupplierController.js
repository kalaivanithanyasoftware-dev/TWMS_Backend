...
const performAmsSupplierAction = async (req, res) => {
    const { globalPlant, plantid: userPlantid, id: userid, roleid, firstname: currentUser, empcode: currentUserEmpcode } = req.userDetails.user
    const {
        globalPlantId,
        plantid,
        totalentry: limit,
        ...
        const { success, error, results: [data = [], [{ count: totalRecords = 0 } = {}] = []] } = (await executeSP([
            {
                ...
                params: [
                    ...
                    { name: 'suppliercode', type: sql.NVarChar, value: suppliercode },
                    { name: 'suppliername', type: sql.NVarChar, value: suppliername },
...
export { performAmsSupplierAction }