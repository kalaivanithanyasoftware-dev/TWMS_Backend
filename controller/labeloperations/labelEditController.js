...
const performLabelEditActions = async (req, res) => {
    ...
    try {
        ...
        if (['MPNList', 'SuppliersList', 'MakeList'].includes(action)) {
            return res.status(201).send({ success, data, action, method });
        }
        if (['UpdateLabel', 'Upload'].includes(action)) {
            if (error) return res.status(201).send({
                notify: true,
                success: false,
                error: true,
...
export { performLabelEditActions }